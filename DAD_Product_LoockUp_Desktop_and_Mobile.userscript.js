// ==UserScript==
// @name         Product LookUp Desktop & Mobile by @MOHAJIHO
// @author    Mohsen Hajihosseinnejad Alias @MOHAJIHO email: mohajiho@gmail.com
// @namespace    https://mohajiho.com/
// @version      1.0
// @description  Finds useful product info and generates QR Code in a new tab
// @match        *://*.amazon.com/*
// @match        *://*.amazon.*/*
...
(function () {
  ...
  const CONFIG = {
    ...
    // Matches “($0.79 / count)”  OR  “($0.79 $0.79 / count)”
  ...
  function buildInnerHTML(data) {
    ...
    data.codes.forEach(code => {
      ...
      if (data.codes.length === 1) {
        ...
        const snapClass =
          data.snapEbtStatus.startsWith('SNAP') ? 'snap-eligible' : 'snap-not-eligible';
  ...
  function scanPage() {
    ...
    const win = window.open('about:blank', '_blank',
      'width=380,resizable=yes,scrollbars=yes');
    ...
    doc.write(`<!doctype html><html><head>
      <meta charset="utf-8"><title>ASIN Finder</title>
      ...
      <script>
        ...
        window.addEventListener('load', () => {
          ...
          try { window.resizeTo(380, h + 20); } catch {/* some browsers block it */}
          if (data.codes.length > 1)
        ...
        data.codes.forEach(c =>
          new QRCode(document.getElementById('qr-' + c), { text:c, width:200, height:200 })
        );
