// ==UserScript==
// @name         DAD Batch Notifier v2 - by Mohajiho
...
let assignableTimestamp = null;  // Timestamp to track when "ASSIGNABLE" first appears
...
function playBeep() {
  ...
  try {
    ...
    fetch(soundUrl)
      .then(response => response.ok ? response.arrayBuffer() : Promise.reject('Failed to load sound file'))
...
function checkAndPlaySound() {
  ...
  if (found) {
    // If "ASSIGNABLE" is found and it's not notified yet
    if (!hasNotified && !assignableTimestamp) {
      // Mark the first appearance time of "ASSIGNABLE"
      assignableTimestamp = Date.now();
      console.log("ASSIGNABLE found! Waiting 5 seconds before notification...");
    }

    // Check if "ASSIGNABLE" has been present for more than 5 seconds
    if (assignableTimestamp && (Date.now() - assignableTimestamp >= 5000)) {
      if (!hasNotified) {
        playBeep();
        hasNotified = true;
        console.log("ASSIGNABLE is confirmed for more than 5 seconds. Starting notification interval");
        ...
        intervalId = setInterval(() => {
          if (document.body.textContent.includes("ASSIGNABLE")) {
            ...
            intervalId = null;
            hasNotified = false;
            assignableTimestamp = null; // Reset timestamp
    ...
    // Reset if "ASSIGNABLE" is no longer found and notifications have been played
    if (hasNotified) {
      clearInterval(intervalId);
      intervalId = null;
      hasNotified = false;
      assignableTimestamp = null; // Reset timestamp when "ASSIGNABLE" disappears
...
function injectStartButton() {
  ...
  button.addEventListener('click', () => {
    ...
    audioContext.resume().then(() => {
      ...
      overlay.remove();
      button.remove();
      checkAndPlaySound();  // Resume monitoring after re-enabling audio
...
// Function to monitor audio context state
function monitorAudioContext() {
  if (audioContext && audioContext.state === 'suspended') {
    console.log("Audio context suspended. Re-enabling notifications...");
    assignableTimestamp = null;  // Reset timestamp
    hasNotified = false;  // Ensure notifications are not in progress
    showStartButtonAgain();  // Show the "Start Notifications" button again
  }
}

// Function to show the "Start Notifications" button again
function showStartButtonAgain() {
  if (document.getElementById('initNotify')) return;  // Avoid showing multiple buttons
  injectStartButton();
}
...
// Start notification interval function
...
function initialize() {
  injectStartButton();
  startObserver();
  // Start monitoring the audio context every second
  setInterval(monitorAudioContext, 1000);  // Check every second
}
